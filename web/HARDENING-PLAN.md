...
| 2.3 | **Skeletons on all 42 routes** | M | Reuse the capsule loader. Respect the `notFound()` rule — segments that 404 must not get a loading boundary. **And check what the segment renders with scripts off** — see 2.8. |
| 2.8 | **A segment with `loading.tsx` renders as nothing but its skeleton without JS** | S | Measured on `/catalog`: with scripts disabled the entire page body stays in React's `display:none` streaming staging div — no `<h1>`, no rows, just the skeleton. The boundary makes the whole page a Suspense child, and only JS swaps the payload in. Googlebot does run JS so this is not an immediate ranking problem, but it means the no-JS and non-executing-crawler view of eight segments is an empty shell. Fix by moving the boundary below the page heading, so the shell that renders without JS still says what the page is. |

...
test.describe('ops console shell', () => {
  test('replaces the marketing header on every ops route', async ({ page }) => {
    ...
    for (const path of ['/en/admin', '/en/admin/products', '/en/admin/users', '/en/admin/market-data']) {
      ...
      await expect(page.getByTestId('ops-shell'), `${path} has no ops shell`).toBeVisible();
  ...
  test('keeps the marketing header on trading pages', async ({ page }) => {
  ...
  test('shows the signed-in account and the pending-review count', async ({ page }) => {
    ...
    const badge = page.getByTestId('ops-nav-admin').locator('span').last();
  ...
  test('marks the current section and only that section', async ({ page }) => {
    ...
    await expect(page.getByTestId('ops-nav-admin-products')).toHaveAttribute('aria-current', 'page');
    // `/admin` must not light up for every page underneath it.
    await expect(page.getByTestId('ops-nav-admin')).not.toHaveAttribute('aria-current', 'page');
  });
  ...
  test('offers each staff role only the sections it may open', async ({ page }) => {
    ...
    await expect(page.getByTestId('ops-nav-admin')).toBeVisible();
    await expect(page.getByTestId('ops-nav-admin-products')).toHaveCount(0);
    await expect(page.getByTestId('ops-nav-admin-users')).toHaveCount(0);

    await page.getByRole('button', { name: 'Sign out' }).click();
    await login(page, 'catalog@pharmalink.global');
    await expect(page.getByTestId('ops-nav-admin-products')).toBeVisible();
    await expect(page.getByTestId('ops-nav-admin')).toHaveCount(0);
  });

  test('nothing stacks or overlaps on scroll', async ({ page }) => {
    await login(page, USERS.admin);
    await page.goto('/en/admin/products');
    const topbar = page.getByTestId('ops-crumb');
